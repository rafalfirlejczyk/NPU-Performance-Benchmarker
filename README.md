...
As a core takeaway, using the correct native accelerator (QNN DSP) yields a remarkable 5x increase (for legacy hardware) to a 13x increase (for flagship silicon) in throughput (FPS). Conversely, calling the NPU in an unsupported configuration triggers an inevitable fallback to the CPU, decreasing performance by 20–35% compared to baseline integer CPU inference.**
---
Frames per Second
